...
import { TRANSAKSI, KURSUS } from "../../../api/API";
...
export default function ListTransaksi() {
  ...
  const getData = useCallback(async () => {
    ...
    const responseKursus = await axios.get(`${KURSUS}`, {
      headers,
    });
    setKursus(responseKursus.data.filter((item) => item.mentorId === userId));
  }, [userId]);
  ...
  return isLoading ? (
    ...
    <Container maxW="7xl" data-aos="fade-up" minH={"70vh"}>
      ...
      <Stack spacing={2} mt={2}>
        {transaksi
          .filter((item) => item.kursusId === kursus[0].id)
          .map((item, index) => (
